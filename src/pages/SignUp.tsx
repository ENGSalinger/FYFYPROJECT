...
export default function SignUp({ onSignUp, onBack, theme, onToggleTheme }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="oled-card animate-slide-up auth-card" style={{ width: '100%', maxWidth: '460px', position: 'relative' }}>
        <div className="scan-overlay" />

        {/* Header */}
        <div style={{ marginBottom: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
            <button onClick={onBack} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '6px', padding: '6px 10px', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: '12px', fontFamily: 'Exo 2', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s' }}>
          ...
          <div className="font-display" style={{ fontSize: '20px', fontWeight: '700', color: 'var(--text-primary)' }}>Register New Vehicle</div>
          <div style={{ color: 'var(--text-secondary)', fontSize: '13px', marginTop: '4px' }}>Your data is saved securely to the TUKBMS database</div>
        </div>

        {/* Progress bar */}
        <div style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}>
          {[1, 2].map(s => (
            <div key={s} style={{ flex: 1, height: '3px', borderRadius: '2px', background: step >= s ? '#00E676' : 'var(--border)', transition: 'background 0.4s ease', boxShadow: step >= s ? '0 0 8px rgba(0,230,118,0.4)' : 'none' }} />
          ))}
        </div>
        <div style={{ color: 'var(--text-secondary)', fontSize: '10px', fontFamily: 'Exo 2', letterSpacing: '0.12em', marginBottom: '22px' }}>
        ...
        {step === 1 ? (
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
                  {[0, 1, 2, 3].map(i => {
                    ...
                    return <div key={i} style={{ flex: 1, height: '3px', borderRadius: '2px', background: i < s ? cols[s - 1] : 'var(--border)', transition: 'background 0.3s' }} />
                  })}
                </div>
                <span style={{ color: 'var(--text-secondary)', fontSize: '10px', fontFamily: 'Exo 2' }}>
            ...
            <div style={{ background: 'rgba(0,184,217,0.05)', border: '1px solid rgba(0,184,217,0.15)', borderRadius: '8px', padding: '10px 12px', display: 'flex', gap: '8px' }}>
              ...
              <span style={{ color: 'var(--text-secondary)', fontSize: '11px', lineHeight: 1.5 }}>
...
function Field({ label, error, hint, children }: { label: string; error?: string; hint?: string; children: React.ReactNode }) {
  return (
    <div>
      <label style={{ display: 'block', color: 'var(--text-secondary)', fontSize: '11px', fontFamily: 'Exo 2', fontWeight: '600', letterSpacing: '0.15em', marginBottom: '8px' }}>{label}</label>
      {children}
      {hint && !error && <span style={{ color: 'var(--text-tertiary)', fontSize: '11px', marginTop: '4px', display: 'block' }}>{hint}</span>}
